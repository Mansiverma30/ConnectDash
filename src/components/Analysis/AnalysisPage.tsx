...
const AnalysisPage: React.FC = () => {
      ...
      const fetchUserData = async () => {
        try {
          const res = await fetch(
            "https://connectdashapi.onrender.com/api/users"
          );
          if (!res.ok) throw new Error("Failed to fetch user");
          const data = await res.json();
          const filteredData = data.find(
            (entry: any) => entry.full_name === fullName
          );
  ...
  return (
    <div className="min-h-screen p-6 bg-[#fff7ed] scroll-mt-24 py-28">
      <div
        className="bg-white rounded-lg shadow-md p-4 mb-6 max-w-3xl mx-auto"
        id="analysis-page"
      >
        <h2 className="text-2xl font-semibold text-[#162020] mb-4">
          Analysis Results
        </h2>
        ...
        <div className="mb-6 p-6 bg-white rounded-xl shadow-md text-[#162020]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <label className="flex items-center gap-3 p-4 bg-pink-50 rounded-lg hover:shadow-md transition-all cursor-pointer border-l-4 border-[#E1306C]">
              ...
              <span className="font-medium text-[#E1306C]">
            ...
            <label className="flex items-center gap-3 p-4 bg-blue-100 rounded-lg hover:shadow-md transition-all cursor-pointer border-l-4 border-[#4267B2]">
              ...
              <span className="font-medium text-[#4267B2]">
      ...
      <div className="flex items-center justify-center">
        <button
          onClick={downloadPDF}
          className="md:w-60 bg-[#ea580c] text-white text-lg font-bold hover:bg-orange-600 transition-all shadow-md hover:shadow-lg py-3 px-6 rounded-lg mt-3 transform hover:scale-105 shadow-[#F97316]/50"
        >
          Download as PDF
        </button>
      </div>
